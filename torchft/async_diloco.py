...
class AsyncDiLoCoServer(ParameterServer):
    ...
    def __init__(
        ...
        # W from the paper: worker_id → (v(w), last_seen_timestamp)
        self._worker_speeds: Dict[str, Tuple[float, float]] = {}
    ...
    def forward(self, session_id: str, pg: ProcessGroup) -> None:
        ...
        Protocol (worker is rank 1, server is rank 0):
          ...
          3. Worker → worker_id: 16-byte UUID as uint8[16] tensor.
          4. If full sync: Worker → pseudo-grads (one tensor per param).
          5. Server → updated global params (one tensor per param).
          6. Server → new_steps scalar: DyLU-recommended local steps
        ...
        # 2. Worker speed for DyLU
        speed_buf = torch.zeros(1)
        pg.broadcast_one(speed_buf, root=1).wait()
        worker_speed = speed_buf[0].item()

        # 3. Worker identity — persistent UUID so the server tracks v(w) per worker
        ...
        if is_full_sync:
            # 4. Receive pseudo-gradients from the worker
            ...
            # Apply outer step and compute DyLU recommendation (serialized).
            # Implements Eq. 6: w.step = floor(v(w) / max_{w'∈W} v(w') * H)
            with self._lock:
                now = time.monotonic()
                if self._dylu_H > 0 and worker_speed > 0:
                    self._worker_speeds[worker_id] = (worker_speed, now)
                # Expire workers that haven't synced within the timeout window
            ...
            # Pull-only: snapshot current global params, return H as hint
            with self._lock:
                snapshot = {
                    name: p.detach().clone()
                    for name, p in self._model.named_parameters()
        ...
        # 5. Send global params back to the worker
        for name in self._param_names:
            pg.broadcast_one(snapshot[name], root=0).wait()

        # 6. Send DyLU recommended steps (0 = disabled)
...
class AsyncDiLoCo:
    ...
    def __init__(
        ...
        # Persistent identity sent to the server on every sync so the server
        # can maintain per-worker speed entries for the DyLU W set.
        _uid = uuid.uuid4()
        self._worker_id_tensor: torch.Tensor = torch.tensor(
            list(_uid.bytes), dtype=torch.uint8
        )
        ...
        if heartbeat_address is not None:
            wid_str = str(uuid.UUID(bytes=bytes(self._worker_id_tensor.tolist())))
            self._heartbeat_url: Optional[str] = (
                f"{heartbeat_address}?worker_id={wid_str}"
            )
        else:
    ...
    def _initial_pull(self) -> None:
        ...
        try:
            ...
            pg.broadcast_one(self._worker_id_tensor, root=1).wait() # worker identity
            new_global: Dict[str, torch.Tensor] = {}
            for name in self._param_names:
                buf = torch.zeros_like(self._global_params[name])
    ...
    def _sync(self) -> None:
        ...
        try:
            ...
            pg.broadcast_one(self._worker_id_tensor, root=1).wait() # worker identity
            for name in self._param_names:
                pg.broadcast_one(pseudo_grads[name], root=1).wait()

            new_global: Dict[str, torch.Tensor] = {}
            for name in self._param_names:
                # zeros_like: shape/dtype correct; value overwritten by broadcast
                buf = torch.zeros_like(self._global_params[name])
                pg.broadcast_one(buf, root=0).wait()
                new_global[name] = buf
